...
class JobPosting:
    ...
    def __repr__(self) -> str:
        ...
        roles_repr = ", ".join([f"{repr(role)}: {repr(link)}" for role, link in self.roles.items()])
        # Ensuring locations are displayed as a list of strings
        locations_repr = ", ".join(repr(location) for location in self.locations)
        return (f"JobPosting(company_name={repr(self.company_name)}, career_site={repr(self.career_site)}, "
