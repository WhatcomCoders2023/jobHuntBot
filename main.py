...
def main(data, context):
    ...
    for repo_name in REPO_NAMES:
        ...
        if latest_jobs_contents:
            ...
            job_service = GithubTableMarkdownParser(
                DUMMY_FILENAME, github_service.last_fetched_timestamp_in_db
            )
    ...
    if job_postings_from_all_repo:
        # TODO figure out why doesn't work in prod
        # channel_id = int(os.getenv("TEST_CHANNEL_ID"))
        channel_id = int(
            os.getenv("WHATCOM_CHANNEL_ID")
        )  # TODO figure out why doesn't work in prod
        job_hunting_bot = JobHuntingBot(
            DUMMY_FILENAME, channel_id, job_postings_from_all_repo
        )
